...
| **India (IN)** | **Run the English rows first and give them most of the budget** — see the India language note below. Hindi (Devanagari): `डिस्ट्रीब्यूटर` (distributor — the loanword, commoner in real trade copy than the Sanskritic form) · `वितरक` (distributor) · `आयातक` (importer) · `थोक विक्रेता` (wholesaler) · `थोक` (wholesale, as a modifier) · `डिस्ट्रीब्यूटरशिप` (distributorship) · `वितरक चाहिए` / `डिस्ट्रीब्यूटर चाहिए` ("distributor wanted" — the intent phrase) · `कॉस्मेटिक्स` (cosmetics, the commercially dominant loanword) / `सौंदर्य प्रसाधन` (cosmetics, the formal register used in official text) · `त्वचा देखभाल` (skincare). Marathi, **low-yield-expected**: `घाऊक विक्रेता` (wholesale seller — `घाऊक` is distinctively Marathi where Hindi uses `थोक`, so it is a useful language discriminator) · `आयातदार` (importer) | **CDSCO** `https://cdsco.gov.in/opencms/opencms/en/Cosmetics/cosmetics/` — **tier 2, plain server-rendered HTML, notice tables in the markup (checked 2026-09-19)**. Its *List of RC Holders* PDF pairs the **Indian agent/importer** with the foreign manufacturer and has a real text layer; it covers **2013–2017 only**, so treat every row as a lead to re-verify on the company's own site, never as a current fact. There is **no current public register**: the live CDSCO online portal exposes no cosmetics query, and the newer notices are individual cancellations/surrenders. · **Cosmoprof India** (Mumbai; BolognaFiere Cosmoprof with Informa Markets India) — its exhibitor list rendered **name + country + booth in plain HTML** when checked 2026-09-19, which makes it the rare directory this playbook can actually harvest, and it tags Korean exhibitors. · **Professional Beauty India** (IDEX Events) — salon/professional channel; **client-rendered, no exhibitor list in the HTML** when checked. · CII member directory — **bot-challenged, not readable**; FICCI, the Indian Chamber of Commerce and KOTRA India publish **no public member/buyer directory**; IBEF is a research portal, not a membership body. · **There is no "Beautyworld India"** — it is not in the organiser's current portfolio; do not cite it. Legal forms `Pvt Ltd`, `Private Limited`, `LLP` are stripped by name normalization; the honorific prefix `M/s.` is **not**, so strip it by hand before comparing names |
| **Indonesia (ID)** | `distributor kosmetik Korea` (Korean cosmetics distributor) · `distributor resmi` (official/authorised distributor) · `distributor tunggal` (sole distributor) · `importir kosmetik` (cosmetics importer) · `grosir kosmetik` (cosmetics wholesale) · `agen kosmetik` (cosmetics agent) · `mencari distributor` ("looking for a distributor") · `peluang kemitraan` (partnership opportunity) · `pengajuan keagenan` (agency application) · `pemilik notifikasi` / `pemegang notifikasi` (notification holder — the BPOM term that names the importer) · `skincare Korea grosir` | **BPOM public product database** `cekbpom.pom.go.id` — the notification holder on a Korean brand's cosmetic record **is** its Indonesian importer, which makes this the highest-value ID source in principle. **Not yet field-tested: a fetch on 2026-09-19 returned the search shell only, and no result page was retrieved**, so whether the holder is in the returned HTML or behind a client-rendered query is unconfirmed. Treat it as unverified until someone reads a result, and respect its terms — open what a search result points at, never enumerate it. · **Cosmobeauté Indonesia** (ICE BSD City, Jakarta; Informa Markets, part of the Cosmoprof network) — the current flagship; exhibitor-list retrievability **not yet field-tested**. · KADIN (chamber) and PERKOSMI (the cosmetics industry association) member listings — **not yet field-tested**. · Halal certifier licensee lists, and **BPJPH's foreign-halal-body list** `https://bpjph.halal.go.id/cari/lembaga-halal-luar-negeri` — a Next.js app whose **body names and recognised scopes were nonetheless present in the returned HTML (checked 2026-09-19)**. Legal forms `PT` and `CV` (leading) and `Tbk` (trailing) are stripped by name normalization |
| **Türkiye (TR)** | `distribütör` (distributor) · `yetkili distribütör` (authorised distributor) · `tek yetkili distribütör` (sole authorised distributor) · `ithalatçı` (importer) · `ithalat` (import) · `toptan` (wholesale) · `toptan kozmetik` (cosmetics wholesale) · `bayilik başvurusu` (dealership application) · `distribütörlük başvurusu` (distributorship application) · `bayilik veren firmalar` (companies granting dealerships) · `Kore kozmetik ithalatçısı` (Korean cosmetics importer) · `cilt bakımı toptan` (skincare wholesale) | **BeautyEurasia** (Istanbul; ICA Events, under the supervision of TOBB) — checked 2026-09-19: the current-edition exhibitor page is a **JS shell** carrying "available soon", but past editions are published as **downloadable exhibitor-list PDFs**, which is exactly the §4 caveat's preferred substitute. Harvest the PDFs. · **TİTCK / ÜTS citizen query** `utsuygulama.saglik.gov.tr/UTS/vatandas` — a public cosmetic-product lookup is documented to exist ("Kozmetik Ürünleri Listele"), and because the regulation requires a **responsible person established in Türkiye** the record should name the local importer. **Not yet field-tested: the host timed out on 2026-09-19**, so neither its retrievability nor the fields it returns are confirmed. · İTO / TOBB chamber member queries — **not yet field-tested**. Note the direction problem: İKMİB, KSD and `turkishexporter.net` index Turkish **exporters**, which is the wrong population; prefer sources that reveal the **import** function (ÜTS responsible person, `ithalatçı` on the company's own site, the customs-facing pages of a distributor). Legal forms `A.Ş.`, `Ltd. Şti.`, `Şti.`, `Anonim Şirketi`, `Limited Şirketi`, `San. ve Tic.`, `Sanayi ve Ticaret` and a trailing `Tic.` are stripped by name normalization |
...
**India is an English-first market for B2B.** The Hindi rows above are a **long-tail supplement**, not a primary channel: Hindi-language queries return consumer listings and "how to start a business" content, while the equivalent English queries return the actual distributor-recruitment and wholesale supply. Marathi trade pages are rarer still, which is why Mumbai is searched in **English and Hindi** and the two Marathi variants are marked low-yield-expected — run them once, log the result, and do not spend budget there before the English rows and the CDSCO/Cosmoprof sources of the India row are exhausted. (Assessment from search behaviour on 2026-09-19, not a measured cohort — if a live run says otherwise, the run wins and this note should be corrected.)

**Regulator registries are the highest-value class in these three markets, and the one this playbook is least sure of.** A cosmetics regulator records who may *import*: India's CDSCO registration names the Indian agent or importer, Indonesia's BPOM notification is held by a locally established notification holder, and Türkiye's ÜTS record carries a responsible person established in Türkiye. That is precisely the fact a buyer-discovery run is trying to establish, and it is stated by the authority rather than by marketing copy. Two honesty rules apply without exception. First, **everything in the three rows above that is not marked as checked on a date is "not yet field-tested"** — it has not been exercised in a live run, and a source that has not been read is not a source (§10). Record the date and the URL you actually read, and correct the row when a run disagrees with it. Second, the §2 retrieval boundary is unchanged: a registry's `robots.txt` and terms of use bind here exactly as they do everywhere else — open what a search result points at, read it, cite it; never enumerate a registry, never page through it programmatically, never mirror it, and never drive a browser at a client-rendered query to get round it. A registry you cannot read leaves the fact `"unknown"`, which is the correct outcome.
