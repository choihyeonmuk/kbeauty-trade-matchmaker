...
**Optional Korean labels.** When the operator asks for Korean output, labels may be swapped using this fixed map; the JSON documents never change: `Summary`→`요약`, `Top Candidates`→`상위 후보`, `Matches`→`매칭 결과`, `Excluded`→`제외`, `Type`→`유형`, `Why`→`근거`, `Contact`→`연락 채널`, `Evidence`→`출처`, `Missing`→`확인 필요`, `Risks`→`위험`, `No qualified match`→`적격 매칭 없음`, `Recommended next action`→`다음 권장 조치`.
