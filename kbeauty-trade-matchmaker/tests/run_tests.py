...
def _diff_properties(report, paths, goldens):
    ...
    report.check("run diff: a confidence delta is computed in Decimal (kantoimport "
                 ...
                 any(i["record_id"] == "BUY-kantoimport-example"
                     and i.get("confidence") == {"before": 0.47, "after": 0.21, "delta": -0.26}
                     for i in asof["changed"]),
...
def _mcp_write_cases(report, root, poison_doc):
    ...
    try:
        ...
        report.check("mcp: M-27 a scorer that fails its own validation leaves output_path "
                     "unwritten, reports the .invalid sibling and says a retry needs a new "
                     "output_path",
                     _mcp_is_error(by_id.get(402)) is True and failed.get("exit_code") == 1
                     and (failed.get("output") or {}).get("path")
                     == os.path.join(root, "out", "poison.scored.invalid.json")
                     and not os.path.lexists(os.path.join(root, "out", "poison.scored.json"))
        ...
        report.check("mcp: M-29 nothing but the requested file and the failed call's .invalid "
                     "sibling was written under the root",
                     sorted(os.listdir(os.path.join(root, "out")))
                     == ["dangle.json", "match.json", "poison.scored.invalid.json"],
